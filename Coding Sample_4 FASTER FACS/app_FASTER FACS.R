...
server <- function(input, output) {
    ...
    df %>%                            # rename first col, which my vary depending on import method
        ...
        mutate(Specimen_ID = map_chr(DIVA_specimen, ~ str_split(.x, "_")[[1]][2])) %>%    # Pull out DIVA "specimen" information
        mutate_at(vars(Specimen_ID), as.numeric) %>%
        arrange(Specimen_ID) %>%
        filter(!grepl('FMO', DIVA_specimen)) %>%
        filter(!DIVA_specimen %in% c("SD", "Mean")) %>%
        mutate(well = map_chr(DIVA_specimen, ~ str_split(.x, "_")[[1]][3])) %>%
        group_by(well) %>%
        mutate(Plate = 1:length(`well`)) %>%
        ungroup() %>%  
        select(Specimen_ID, Plate, well, DIVA_specimen, everything()) %>%
        separate(well, c("well_row", "well_col"), sep = 1) %>%
        gather(key = "full_gating_path", value = "readout", 6:ncol(.)) %>%
        mutate(plot_label = str_extract(full_gating_path, "(?<=\\|)[^\\/]+")) %>%
        mutate(plot_label = gsub(pattern = "Mean", replacement = "MFI", x = .$plot_label)) %>%
        mutate(plot_label = gsub(pattern = "\\(", replacement = "", x = .$plot_label)) %>%
        mutate(plot_label = gsub(pattern = "\\)", replacement = "", x = .$plot_label)) %>%
        mutate(Gate = str_extract(full_gating_path, "(?<=\\/)[^\\/]+(?=\\|)")) %>%
        mutate(ParentGate = str_extract_all(.$full_gating_path, regex("(?<=\\/)[^\\/]+(?=\\/)", multiline = TRUE))) %>%
        mutate(ParentGate = unlist(unique(map(.[,"ParentGate"], function(x) {map(x, last)})))) %>%
        mutate(GrandparentGate = map_chr(full_gating_path, ~ nth(flatten(str_split(.x, "/")), -3))) %>%
        mutate(plot_label =  str_replace_all(.$plot_label, "Freq. of Parent %", paste0(Gate, " (% freq. of ", ParentGate, ")"))) %>%
        mutate(plot_label = gsub(pattern = "HLA ", replacement = "HLA\\-", x = .$plot_label)) %>%
        select("Specimen_ID", "DIVA_specimen", "Plate", "well_row", "well_col", "full_gating_path", "GrandparentGate", "ParentGate", "Gate", "plot_label", "readout", everything())     #"full_gating_path",  "well_row", "well_col", "plot_label", "ParentGate", "Gate", "readout",
  ...
  output$table_long <- renderTable({   # show data set based on ui selection and data match in server/dataInput()
